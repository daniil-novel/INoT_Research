...
- В репозиторий добавлены артефакты реального E6-запуска в `results/e6`.
- В README добавлен раздел `Исследования` с выводами по E6.
- Hard budget проекта снижен до `$10` под текущий экспериментальный лимит.
- Локальный `.env` теперь имеет приоритет над системной переменной окружения,
  чтобы случайно не использовать старый OpenRouter-ключ.
- Модели по умолчанию заменены на Gemini:
  `google/gemini-3.1-pro-preview` и `google/gemini-3.1-flash-lite`.
- Добавлен эксперимент E6 для сравнения Pro и Flash-Lite внутри B2 и B3.
